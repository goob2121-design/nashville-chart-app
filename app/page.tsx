...
function isReferenceTag(line: string) {
  return /^\[(?:Same as .+|V2 = V1|V3 = V1|Ch2 = Ch1|Solo = Verse|Break = Chorus|Outro = Intro|Last Line Chorus|Last 2 Lines Chorus|Verse Chords|Chorus Chords|Kick on 5|Stop on 1|Build|Half-time|Walk Up|N\.C\.|Cold End|Fade|x2|x3|Banjo Kickoff|Fiddle Kickoff|Dobro Solo|Mando Chop In|Harmony In|Tag Last Line x2)\]$/i.test(
    line.trim()
  );
}
    ...
    <main className="min-h-screen bg-zinc-950 px-4 py-8 text-zinc-100 sm:px-6 sm:py-12 print:bg-white print:px-0 print:py-0 print:text-black">
      ...
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-6">
        ...
        <section className="rounded-3xl border border-zinc-800 bg-black/70 p-6 shadow-xl shadow-black/20 print:rounded-none print:border-0 print:bg-white print:p-0 print:shadow-none">
          ...
          {chart.notes.trim() ? (
            <section className="mt-5 space-y-2 print:mt-4">
              <h3 className="text-sm font-medium uppercase tracking-[0.2em] text-zinc-400 print:text-black">Notes</h3>
              <p className="whitespace-pre-wrap text-sm leading-7 text-zinc-200 print:text-black">{chart.notes}</p>
            </section>
          ) : null}
          ...
          <section className="mt-5 space-y-2 print:mt-4">
            <h3 className="text-sm font-medium uppercase tracking-[0.2em] text-zinc-400 print:text-black">Nashville Chart</h3>
      ...
      <main className="min-h-screen bg-zinc-950 px-4 py-8 text-zinc-100 sm:px-6 sm:py-12 print:bg-white print:px-0 print:py-0 print:text-black">
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 print:max-w-none print:gap-4">
          <div className="no-print space-y-2">
            <p className="text-sm uppercase tracking-[0.3em] text-zinc-400">Nashville Number System</p>
          ...
          <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr] print:grid-cols-1">
            <section className="no-print space-y-5 rounded-3xl border border-zinc-800 bg-zinc-900/80 p-5 shadow-xl shadow-black/20">
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="flex flex-col gap-2 text-sm font-medium text-zinc-200">
                  Song Title
                  <input className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-base text-white outline-none transition focus:border-zinc-500" value={songTitle} onChange={(event) => setSongTitle(event.target.value)} />
                </label>
                <label className="flex flex-col gap-2 text-sm font-medium text-zinc-200">
                  Artist
                  <input className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-base text-white outline-none transition focus:border-zinc-500" value={artist} onChange={(event) => setArtist(event.target.value)} />
                </label>
              </div>
              ...
              <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)]">
                <label className="flex flex-col gap-2 text-sm font-medium text-zinc-200">
                  Concert Key
                  <select className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-base text-white outline-none transition focus:border-zinc-500" value={selectedKey} onChange={(event) => setSelectedKey(event.target.value as KeyName)}>
                    {MAJOR_KEYS.map((key) => <option key={key} value={key}>{key}</option>)}
                  </select>
                </label>
                <label className="flex flex-col gap-2 text-sm font-medium text-zinc-200">
                  Transpose To Key
                  <select className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-base text-white outline-none transition focus:border-zinc-500" value={transposeToKey} onChange={(event) => setTransposeToKey(event.target.value as KeyName)}>
                    {MAJOR_KEYS.map((key) => <option key={key} value={key}>{key}</option>)}
                  </select>
                </label>
                <label className="flex flex-col gap-2 text-sm font-medium text-zinc-200">
                  Time Signature
                  <select className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-base text-white outline-none transition focus:border-zinc-500" value={timeSignature} onChange={(event) => setTimeSignature(event.target.value as TimeSignature)}>
              ...
              <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1.4fr)]">
                <label className="flex flex-col gap-2 text-sm font-medium text-zinc-200">
                  Tempo
                  <input className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-base text-white outline-none transition focus:border-zinc-500" placeholder="120 BPM" value={tempo} onChange={(event) => setTempo(event.target.value)} />
                </label>
                <label className="flex flex-col gap-2 text-sm font-medium text-zinc-200">
                  Capo
                  <input className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-base text-white outline-none transition focus:border-zinc-500" inputMode="numeric" placeholder="0" value={capo} onChange={(event) => setCapo(event.target.value.replace(/[^\d]/g, ''))} />
                </label>
                <label className="flex flex-col gap-2 text-sm font-medium text-zinc-200">
                  Feel
                  <select className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-base text-white outline-none transition focus:border-zinc-500" value={feel} onChange={(event) => setFeel(event.target.value)}>
                ...
                <div className="rounded-2xl border border-zinc-800 bg-zinc-950/80 p-4 text-sm text-zinc-300">
                  <p><span className="font-medium text-zinc-100">Concert Key:</span> {selectedKey}</p>
                  <p><span className="font-medium text-zinc-100">Capo:</span> {capo || '0'}</p>
                  <p><span className="font-medium text-zinc-100">Play In:</span> {playInKey}</p>
                </div>
              </div>
              ...
              <label className="flex flex-col gap-2 text-sm font-medium text-zinc-200">
                Notes
                <textarea className="min-h-24 rounded-xl border border-zinc-700 bg-zinc-950 px-3 py-3 text-sm leading-6 text-white outline-none transition focus:border-zinc-500" value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Arrangement notes, solos, endings, or reminders for the band." />
              </label>
              ...
              <section className="space-y-3">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button type="button" className="rounded-lg border border-zinc-700 px-3 py-2 text-sm font-medium text-zinc-200 transition hover:bg-zinc-800" onClick={handleSmartPaste}>
                      Smart Paste / Extract Chords
                    </button>
                    <button type="button" className="rounded-lg border border-zinc-700 px-3 py-2 text-sm font-medium text-zinc-200 transition hover:bg-zinc-800" onClick={handleCleanUpInput}>
                      Clean Up Input
                    </button>
                    <button type="button" className="rounded-lg border border-zinc-700 px-3 py-2 text-sm font-medium text-zinc-200 transition hover:bg-zinc-800" onClick={handleTransposeChart}>
                      Transpose Chart
                    </button>
                  </div>
                </div>
                <textarea className="min-h-64 w-full rounded-2xl border border-zinc-700 bg-zinc-950 px-3 py-3 font-mono text-base text-white outline-none transition focus:border-zinc-500" value={input} onChange={(event) => setInput(event.target.value)} onBlur={() => setInput(normalizeChartInput(input))} spellCheck={false} />
                {smartPasteMessage ? <p className="text-sm text-zinc-400">{smartPasteMessage}</p> : null}
              </section>

              <div className="flex flex-wrap gap-2">
                <button type="button" className="rounded-xl bg-amber-400 px-4 py-3 text-sm font-semibold text-zinc-950 transition hover:bg-amber-300" onClick={handleConvert}>
                  Convert to Nashville Numbers
                </button>
                <button type="button" className="rounded-xl border border-zinc-700 px-4 py-3 text-sm font-medium text-zinc-200 transition hover:bg-zinc-800" onClick={handleSaveChart}>
                  Save Chart
                </button>
              </div>
              ...
              <section className="space-y-3 rounded-2xl border border-zinc-800 bg-zinc-950/80 p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  {hasMounted ? (
                    <div className="flex flex-wrap gap-2">
                      <button type="button" className="rounded-lg border border-zinc-700 px-3 py-2 text-sm font-medium text-zinc-200 transition hover:bg-zinc-800 disabled:opacity-50" onClick={handleLoadChart} disabled={!selectedSavedChartId}>
                        Load Chart
                      </button>
                      <button type="button" className="rounded-lg border border-zinc-700 px-3 py-2 text-sm font-medium text-zinc-200 transition hover:bg-zinc-800 disabled:opacity-50" onClick={handleDeleteChart} disabled={!selectedSavedChartId}>
                        Delete Chart
                      </button>
                    </div>
                  ) : (
                ...
                {hasMounted ? (
                  <select className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-base text-white outline-none transition focus:border-zinc-500" value={selectedSavedChartId} onChange={(event) => setSelectedSavedChartId(event.target.value)}>
                    <option value="">Select a saved chart</option>
                    {savedCharts.map((chart) => <option key={chart.id} value={chart.id}>{chart.id}</option>)}
                  </select>
                ) : (
                  <div className="rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-base text-zinc-500">
            ...
            <section className="rounded-3xl border border-zinc-800 bg-black/70 p-5 shadow-xl shadow-black/20 print:rounded-none print:border-0 print:bg-white print:p-0 print:shadow-none">
              <div className="space-y-3 border-b border-zinc-800 pb-4 print:border-zinc-300">
              ...
              <div className="mt-5 space-y-4 print:mt-4 print:space-y-5">
                <div className="no-print flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <h3 className="text-sm font-medium text-zinc-300">Nashville Number Chart</h3>
                  <div className="flex flex-wrap gap-2">
                    <button type="button" className="rounded-lg border border-zinc-700 px-3 py-2 text-sm font-medium text-zinc-200 transition hover:bg-zinc-800" onClick={handleCopyChart}>{copyLabel}</button>
                    <button type="button" className="rounded-lg border border-zinc-700 px-3 py-2 text-sm font-medium text-zinc-200 transition hover:bg-zinc-800" onClick={handleShareView}>{shareLabel}</button>
                    <button type="button" className="rounded-lg border border-zinc-700 px-3 py-2 text-sm font-medium text-zinc-200 transition hover:bg-zinc-800" onClick={() => setPerformanceMode(true)}>Performance Mode</button>
                    <button type="button" className="rounded-lg border border-zinc-700 px-3 py-2 text-sm font-medium text-zinc-200 transition hover:bg-zinc-800" onClick={() => window.print()}>Print Chart</button>
                  </div>
                </div>

                {shareUrl ? (
                  <a href={shareUrl} className="no-print block break-all rounded-2xl border border-zinc-800 bg-zinc-950/70 px-4 py-3 text-sm text-zinc-300">
                    {shareUrl}
                  </a>
                ) : null}

                <section className="no-print space-y-3 rounded-2xl border border-zinc-800 bg-zinc-950/80 p-4">
                  <div className="space-y-1">
                    <h4 className="text-sm font-medium text-zinc-200">Symbol Toolbar</h4>
                    <p className="text-xs text-zinc-400">Insert common Nashville chart marks at the cursor position.</p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {SYMBOL_BUTTONS.map((symbol) => (
                      <button
                        ...
                        type="button"
                        className="rounded-lg border border-zinc-700 px-3 py-2 font-mono text-sm text-zinc-200 transition hover:bg-zinc-800"
                ...
                <textarea
                  ref={outputRef}
                  className="no-print min-h-80 w-full rounded-2xl border border-zinc-800 bg-zinc-950 px-4 py-4 font-mono text-lg leading-8 text-emerald-300 outline-none transition focus:border-zinc-500"
                  value={output}
                  onChange={(event) => setOutput(event.target.value)}
                  spellCheck={false}
                />

                <div className="print-only print-chart-text text-black">
                  <ChartLines text={printChartText} />
                </div>

                {notes.trim() ? (
                  <section className="rounded-2xl border border-zinc-800 bg-zinc-950/80 p-4 text-sm text-zinc-300 print:border-0 print:bg-white print:p-0 print:text-black">
                    <h4 className="mb-3 text-sm font-medium uppercase tracking-[0.2em] text-zinc-100 print:text-black">Notes</h4>
                    <p className="whitespace-pre-wrap leading-7">{notes}</p>
                  </section>
                ) : null}

                <section className="no-print rounded-2xl border border-zinc-800 bg-zinc-950/80 p-4 text-sm text-zinc-300">
                  <h4 className="mb-3 text-sm font-medium text-zinc-100">Symbol Help</h4>
